define([], function () {
    return ["$scope", "$state", function ($scope, $state) {
        ...
        $scope.getValidateCode = function () {
            console.log($scope.info.phoneNumber);
            $scope.validateA = Math.floor(Math.random() * (9999 - 1000)) + 1000
            console.log($scope.validateA)
            if ($scope.info.phoneNumber && document.getElementById("duda").innerText == "获取验证码") {
                $scope.textdudu = "60s";
                _post({
                    url: "/user/sendCode",
                    param: {
                        code: $scope.validateA,
                    ...
                    callback: function (res) {
                        ...
                        bb = setInterval(function () {
                            ...
                            document.getElementById("duda").innerText = ii + "s"
                        }, 1000)

                    }
                })
            } else if (!$scope.info.phoneNumber) {
                alert("请输入手机号")
            }
        ...
        $scope.confirm = function () {
            ...
            if (!$scope.info.userName) {
                ...
                alert("手机号不能为空")
            } else if (!$scope.info.password && $scope.info.password.length < 6) {
                alert("密码不能为空或者长度小于6位数")
            } else if ($scope.validateA != $scope.info.validate) {
                alert("验证码不正确")
            } else if (!$scope.info.province) {
                alert("省份不能为空")
            } else if (!$scope.info.city) {
                alert("城市不能为空")
            } else if (!$scope.info.hospitalName) {
                alert("医院名称不能为空")
            } else if (!$scope.info.jobName) {
                alert("科室不能为空")
            } else if (!$scope.info.title) {
                alert("职称不能为空")
            } else {
                window.clearInterval(bb);
                _post({
